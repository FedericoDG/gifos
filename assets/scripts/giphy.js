...
const renderSearch = (array, start, end) => {
  ...
  for (let index = start; index < array.length; index++) {
    if (index <= end) {
      ...
      /*  */
      divDownload.addEventListener('click', () => download(array[index].images.original.url, array[index].title));
...
const renderTrending = (array) => {
  ...
  array.forEach(element => {
    ...
    divActionsButtons.appendChild(divDownload);
    divDownload.addEventListener('click', () => download(element.url, element.title));
    divActionsButtons.appendChild(divMax);
...
/* OBTENER Y RENDERIZAR FAVORITOS */
...
const renderFavoritesOrMyGifos = (array, start, end, node) => {
  ...
  for (let index = start; index < array.length; index++) {
    if (index <= end) {
      ...
      divDownload.setAttribute('class', 'download');
      divActionsButtons.appendChild(divDownload);
      divDownload.addEventListener('click', () => download(array[index].url, array[index].title));
      const divMax = document.createElement('div');
      divMax.setAttribute('class', 'max');
      divActionsButtons.appendChild(divMax);
...
const max = (id, url, username, title, type) => {
  ...
  downloadPopup.addEventListener('click', () => download(url, title));
};
